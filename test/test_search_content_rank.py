...
def test_data_fixture():
    ...
    doc1 = File.objects.create(
        ...
        tsFilename=SearchVector(Value("Document 1 - Ranking"), weight="A", config='simple'),
        tsContent=SearchVector(Value("I like big burgers"), weight="B", config='english'),
    )
    doc2 = File.objects.create(
        ...
        createdAt=timezone.now(),
        modifiedAt=timezone.now(),
        tsFilename=SearchVector(Value("Document 2 - Ranking"), weight="A", config='simple'),
        tsContent=SearchVector(Value("Mega like mega burgers"), weight="B", config='english'),
    )
    doc3 = File.objects.create(
        ...
        createdAt=timezone.now(),
        modifiedAt=timezone.now(),
        tsFilename=SearchVector(Value("Document 3 - Ranking"), weight="A", config='simple'),
    ...
    doc4 = File.objects.create(
        ...
        createdAt=timezone.now(),
        modifiedAt=timezone.now(),
        tsFilename=SearchVector(Value("Document 4 - Ranking"), weight="A", config='simple'),
        tsContent=SearchVector(Value("Like burgers big"), weight="B", config='english'),
    )

    return {
        "user": user, "service": service, "doc1": doc1, "doc2": doc2, "doc3": doc3, "doc4": doc4,
    }
...
def test_user_search_for_exact_match(test_data):
    ...
    assert_files_appear_in_specified_order(
        ...
        ordered_files=[test_data["doc2"], test_data["doc3"], test_data["doc1"], test_data["doc4"]],
...
def test_user_search_for_partial_match(test_data):
    ...
    assert_files_appear_in_specified_order(
        query="like burgers",
        ordered_files=[test_data["doc1"], test_data["doc4"], test_data["doc2"], test_data["doc3"]],
        base_filter=Q(serviceId=test_data["service"])
    )
